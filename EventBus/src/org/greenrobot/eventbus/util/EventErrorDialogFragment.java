...
public class EventErrorDialogFragment {
    ...
    public static Dialog createDialog(Context context, Bundle arguments, OnClickListener onClickListener) {
        ...
        builder.setTitle(arguments.getString(EventErrorDialogManager.KEY_TITLE));
        builder.setMessage(arguments.getString(EventErrorDialogManager.KEY_MESSAGE));
    ...
    public static void handleOnClick(DialogInterface dialog, int which, Activity activity, Bundle arguments) {
        if (EVENT_TYPE_ON_CLICK != null) {
            ...
            EventBus eventBus = EventErrorDialogManager.factory.config.getEventBus();
            eventBus.post(event);
        }
        boolean finish = arguments.getBoolean(EventErrorDialogManager.KEY_FINISH_AFTER_DIALOG, false);
